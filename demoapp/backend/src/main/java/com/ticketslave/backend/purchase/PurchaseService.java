...
public class PurchaseService {
    ...
    @Transactional
    public void createPurchase(List<Ticket> cart, String userEmail) throws AbortedException{
        Purchase purchase = new Purchase();
        for (Ticket t: cart) {
            if (t.isSold() || !t.getStatus()) {
                throw new AbortedException("Purchase aborted, invalid ticket");
            }
        }
        if (userEmail == null){
            throw new AbortedException("Purchase aborted, invalid user email");
        }
        for (Ticket t: cart){
            t.setSold(true);
            purchase.addTicket(t);
        }
